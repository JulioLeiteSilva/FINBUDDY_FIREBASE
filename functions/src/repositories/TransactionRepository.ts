...
export class TransactionRepository {
  static async createMany(uid: string, transactions: Transaction[]): Promise<void> {
  ...
  static async get(uid: string, transactionId: string): Promise<Transaction | null>{
    const ref = db
      .collection("users")
      .doc(uid)
      .collection("transactions")
  ...
  static async findByDateAndName(uid: string, name: string, date: Date):Promise<Transaction | null> {
    const snapshot = await db
      .collection("users")
      .doc(uid)
      .collection("transactions")
